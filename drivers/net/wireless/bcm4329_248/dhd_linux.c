...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	/* for puccini sigle band hardware XA, XB */
	if (system_rev == 0 || system_rev == 1) {
		if (strstr(firmware_path, "fw_bcm4329.bin")) {
			strcpy(firmware_path, "/system/etc/firmware/fw_bcm4329.bin.xb");
		}
		else {
			strcpy(firmware_path, "/system/etc/firmware/fw_bcm4329_apsta.bin.xb");
		}
		strcpy(nvram_path, "/system/etc/calibration.xb");
	}
	...
	WAKE_LOCK_INIT(&dhd->pub, WAKE_LOCK_LINK_TMOUT, "dhd_wake_lock_link_event");
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	setbit(dhdp->eventmask, WLC_E_ASSOCREQ_IE);
#ifdef PNO_SUPPORT
...
dhd_detach(dhd_pub_t *dhdp)
{
	...
	/* && defined(DHD_GPL) */
		...
		WAKE_LOCK_DESTROY(dhdp, WAKE_LOCK_LINK_TMOUT);
...
init_retry:
	...
	if (down_timeout(&wifi_control_sem,  msecs_to_jiffies(5000)) != 0) {
		printf("%s: platform_driver_register timeout\n", __FUNCTION__);
		/* renove device */
		wifi_del_dev();
		goto faild;
	}
